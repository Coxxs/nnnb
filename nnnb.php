...
class NNNB {
	...
	// char
	private $cr = ['n', 'N', 'Ń', 'ń', 'Ņ', 'ņ', 'Ň', 'ň', 'Ɲ', 'ƞ', 'Ñ', 'Ǹ', 'ǹ', 'Ƞ', 'ȵ'];
	private $cc = ['n', 'N', 'Ń', 'ń', 'Ņ', 'ņ', 'Ň', 'ň', 'Ɲ', 'ƞ', 'Ñ', 'Ǹ', 'ǹ', 'Ƞ', 'ȵ'];
	...
	private function _encodeByte(int $i) : string {
		if ($i > 0xFF) throw new Exception('nn/nb overflow');
		if ($i > 0x7F) {
			$i = $i & 0x7F;
	...
	private function _encodeShort(int $i) : string {
		if ($i > 0xFFFF) throw new Exception('nnnb overflow');
		$reverse = false;
		if ($i > 0x7FFF) {
			$reverse = true;
			$i = $i & 0x7FFF;
	...
	private function _decodeByte(string $c) : int {
		...
		if ($idx[0] === false || $idx[1] === false) throw new Exception('not nn/nb');
		$result = $nb ? $idx[0] * $this->sb + $idx[1] : $idx[0] * $this->sc + $idx[1];
		if ($result > 0x7F) throw new Exception('nn/nb overflow');
	...
	private function _decodeShort(string $c) : int {
		...
		if ($idx[0] === false || $idx[1] === false || $idx[2] === false || $idx[3] === false) throw new Exception('not nnnb');
		$result = $idx[0] * $this->scnb + $idx[1] * $this->snb + $idx[2] * $this->sb + $idx[3];
		if ($result > 0x7FFF) throw new Exception('nnnb overflow');
	...
	public function decode(string $str) : string {
		...
		$output = '';
		// decode every 2 bytes (1 nnnb = 2 bytes)
		for ($i = 0; $i < ($len >> 2); $i++) {
